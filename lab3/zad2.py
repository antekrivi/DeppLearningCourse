...
y = torch.sin(x**2) + 3*x**3

plt.plot(x.cpu(), y.cpu(), '-r', label='y=sin(x**2) + 3*x**3')
plt.title('Graph of y=sin(x**2) + 3*x**3')
...
for t in range(2001):
    ...
    if t % 500 == 0:
        ...
        plt.plot(x.cpu().detach(), y_graph.cpu().detach(), '-r', label='Prediction')
